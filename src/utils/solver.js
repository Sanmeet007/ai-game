...
const isSolvable = (board) => {
  const flatBoard = board.flat().filter((num) => num !== 0); // Exclude empty tile
  let inversions = 0;
  ...
  // For a 3x3 puzzle, the number of inversions must be even for solvability
...
export const shufflePuzzle = (board, moves = 20) => {
  let newBoard = JSON.parse(JSON.stringify(board));
  let attempts = 0;
  const maxAttempts = 10; // Prevent infinite loops

  // Keep shuffling until a solvable puzzle is found
  do {
    newBoard = JSON.parse(JSON.stringify(board)); // Reset to initial board
    ...
    attempts++;
  } while (!isSolvable(newBoard) && attempts < maxAttempts);

  // If no solvable puzzle is found, return the last attempt
  return newBoard;
};
